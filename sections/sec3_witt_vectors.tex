...
\begin{bigproof}
    ...
    \end{claim*}
    \begin{smallproof}
        First, note that $v_p(p^v-i) = v_p(i).$
...
\begin{bigproof}
    ...
    Now
    \begin{align}
        \label{eq: congruence 1} \tag{Eq.1}
        \sum_{d \mid n/p} d \cdot \phi(a_d^{n/pd}) 
        &\equiv
        \sum_{d \mid n/p} d \cdot a_d^{n/d} &\text{\hskip8mm mod} \ p^{v_p(n)}A \\
        \label{eq: congruence 2} \tag{Eq.2}
...
\begin{theorem} \label{thm: existence of Witt vectors}
    There exists a unique ring structure on $\W_S(A)$ such that the ghost map 
    \[
      w:\W_S(A) \to A^s  
    \]
...
\begin{bigproof}
    ...
    = \overline{f^p}$ in $\Fp[a_n,b_n \mid n \in S]$, which is apparent). Let $a = (a_n)_n,b=(b_n)_n$.
    \begin{claim*}
    ...
    \underline{Step 2:} Now let $A'$ be any ring (Remember that $A = \Z[a_n,b_n \mid n \in S]$).
    ...
    \begin{align*}
        ...
        -a' &:= \W_S(e)(I) = (I_n(a'_1,\dots,a'_n))_{n \in S}
    \end{align*}
...
\begin{align*}
    w(a'+b') &= w(\W_S(e)(S)) = e^S(w(S)) = e^S(w(a)+w(b)) \\
    &= e^S(w(a)) + e^S(w(b)) = w(a') +w(b')
\end{align*}
...
\begin{beweis}
    ...
    and $(1,1,1,\dots)$ are the zero element respectively the unit in $A^{\N}$.
...
\begin{bigproof}
    ...
    \begin{itemize}
        \item case 1: $m \neq n \cdot d$ $\forall d \in S/n$: Then
        ...
        $m =k \cdot d' = n \cdot d \cdot d'$ for $d,d' \in S/n$ and then $d \cdot d' \mid m$
        which is a contradiction to case 1.
        \item case 2: $m = n \cdot d$ for some $d \in S/n$:
        \begin{align*}
...
\end{lemma}
    ...
    \[
        \begin{tikzcd}
            \W_S(A) \arrow[d,"F_n"] \arrow[dr,"w_{nd}",sloped] \\
...
\begin{beweis}
    ...
    Then $S/n = \{1\}$ and therefore we have a short exact sequence of $\Z$-modules
    \[
    \begin{tikzcd}
        0 \arrow[r]
          & A \arrow[r,"V_n"]
...
\begin{beweis}
    ...
    Then \[
        F_p(a') = \W e (F_p(a)) = \W e(a^p + p \cdot r) = (a')^p + p \cdot \W e(r)
    \]
    for some $r \in A$.
\end{beweis}
...
\begin{proposition} \label{prop: existence of diagonal}
    ...
    \]
    such that $w_n(\Delta(a))=F_n(a)$ for all $a \in \W(A), n \in \N$.
\end{proposition}
\begin{beweis}
...
\begin{bigproof}
    ...
    \begin{smallproof}
        ...
        using the fact that 
        \begin{tikzcd}
            \W(\W(A)) \arrow[d,"w",dotted] \arrow[rd,"w_{nm}",dotted,sloped]\\
            \W(A)^{\N} \arrow[r,"(\widetilde{F}_n)_{\W(A)}",dotted]
            & \W(A)^{\N}
        \end{tikzcd}
        ...
        %\phantomsection\label{name1}
        \begin{tikzcd}
            \W(A) \arrow[r,"F_n"] \arrow[d,"\Delta_A"] \arrow[rd,"F_{nm}",dotted,sloped]
                 & \W(A) \arrow[d,"F_m"] \\
                \W(\W(A)) \arrow[r,"w_{nm}"] 
                  & \W(A)
        \end{tikzcd}
        ...
        %\phantomsection\label{name2}
        \begin{tikzcd}
            \W(A) \arrow[r,"F_n"] \arrow[rd,"F_{nm}"',sloped]
            & \W(A) \arrow[d,"F_m"]\\
            & \W(A)
        \end{tikzcd}
        %\caption*{(cap2)}
    ...
    \begin{claim*}
        \begin{tikzcd}
            \W(A) \arrow[d,"\Delta_A"'] \arrow[rd,"\id_{\W(A)}",sloped]\\
            \W(\W(A)) \arrow[r,"\W(w_1)"']
            & \W(A) 
        \end{tikzcd}
        commutes.
    ...
    %\phantomsection\label{name2}
    \begin{tikzcd}
        \W(A) \arrow[d,"F_n"'] \arrow[dr,"w_n",sloped]\\
        \W(A) \arrow[r,"w_1"'] 
        & A
      \end{tikzcd}
    %\caption*{(cap2)}
    ...
    \begin{claim*}
        \begin{tikzcd}
            & \W(A) \arrow[d,"\Delta_A"] \arrow[ld,"\id_{\W(A)}",sloped] \\
...
\]
(see Example~\ref{ex: free monoid ring}).
